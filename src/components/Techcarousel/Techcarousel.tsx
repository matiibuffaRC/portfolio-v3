...
function TechCarousel({
    ...
    return (
        <div className="relative w-full overflow-hidden py-3 md:py-4 rounded-2xl bg-gray-100 dark:bg-gray-800">
            ...
            <div
                className={`flex w-max gap-6 md:gap-10 justify-center items-center tech-track ${
                    direction === "left" ? "tech-track-left" : "tech-track-right"
                }`}
                    ...
                    >
                        {technologies.map((tech) => (
                            <img
                                ...
                                title={tech.name}
                                alt={tech.name}
                                src={getIconSrc(tech, dark)}
                                className="h-10 w-10 md:h-14 md:w-14 shrink-0 transition-transform duration-300 hover:scale-110"
                            />
                        ))}
                    </div>
                ))}
...
function TechCarousels({ dark }: { dark: boolean }) {
    ...
    return (
        <div
            className="flex flex-col items-center justify-center gap-3 md:gap-4 w-full max-w-7xl mx-auto px-4 md:px-0"
