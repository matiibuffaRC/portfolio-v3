...
function Header({ open, setOpen, dark, setDark }: HeaderProps) {
  ...
  const linkClass = (id: string) =>
    `hover:bg-gray-200 dark:hover:bg-[#252C34] transition-all duration-700 py-1 px-4 rounded-3xl ${
      activeSection === id
        ? "bg-[#E3F6F5] dark:bg-[#252C34] text-[#176b68] dark:text-[#D1F7F3]"
        : ""
    }`;
    ...
    <header className="bg-gray-100 dark:bg-[#151B23] h-17.5 flex justify-between items-center py-3 px-4 md:px-6 border-b border-gray-300 dark:border-gray-800 fixed w-full z-10">
      ...
      <div className="flex flex-row gap-2">
        <nav
          ...
          className="dark:text-[#D1D7E0] text-[#151B23] urbanist font-semibold text-md hidden md:flex gap-2"
        >
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`w-11 h-6 flex items-center rounded-full p-1 transition-colors duration-300 cursor-pointer ${dark ? "bg-gray-800" : "bg-gray-300"}`}
          >
            <div
              className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-300 ${dark ? "translate-x-5" : "translate-x-0"}`}
            />
          </button>
          ...
          >
            <span
              className={`absolute left-0 w-full h-0.5 dark:bg-[#D1D7E0] bg-black transition-all duration-300 ${open ? "top-1/2 -translate-y-1/2 rotate-45" : "top-1"}`}
            />
            <span
              className={`absolute left-0 w-full h-0.5 dark:bg-[#D1D7E0] bg-black transition-all duration-300 ${open ? "opacity-0" : "top-1/2 -translate-y-1/2"}`}
            />
            <span
              className={`absolute left-0 w-full h-0.5 dark:bg-[#D1D7E0] bg-black transition-all duration-300 ${open ? "top-1/2 -translate-y-1/2 -rotate-45" : "bottom-1"}`}
            />
          </button>
        </div>
      </div>
